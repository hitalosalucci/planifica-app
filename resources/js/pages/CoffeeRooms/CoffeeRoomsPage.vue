<template>
  ...
  <StandardTable :rows="eventsList" :columns="columns" row-key="id" :loading="isLoadingData">
    <template #table-top-right>
      <StandardButton label="Nova Sala de café" buttonClass="brand-button-secondary" leftIcon="add" @click="openCreateDialog" />
    </template>
...
const onSubmit = async () => {
  ...
  try{
    ...
    if (isEdit.value) {
      const endpoint = `/coffee-rooms/${formModel.value.id}`;
      response = await api.put(endpoint, formModel.value);
    } else {
      response = await api.post('/coffee-rooms', formModel.value);
    }
